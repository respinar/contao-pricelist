<?php

/**
 * Contao Open Source CMS
 *
 * Copyright (c) 2005-2014 Leo Feyer
 *
 * @package   ProductList
 * @author    Hamid Abbaszadeh
 * @license   GNU/LGPL
 * @copyright 2014
 */


/**
 * Fields
 */
$GLOBALS['TL_LANG']['tl_pricelist_item']['title']        = array('Title', 'Please enter the item title.');
$GLOBALS['TL_LANG']['tl_pricelist_item']['code']         = array('Code (Model)', 'Please enter the item code or model.');
$GLOBALS['TL_LANG']['tl_pricelist_item']['unit']         = array('Unit', 'Please enter the unit.');
$GLOBALS['TL_LANG']['tl_pricelist_item']['stock']        = array('Stock', 'Product exist in stock.');
$GLOBALS['TL_LANG']['tl_pricelist_item']['sale']         = array('Sale', 'Product is in sale.');
$GLOBALS['TL_LANG']['tl_pricelist_item']['url']          = array('URL', 'Please enter the URL.');
$GLOBALS['TL_LANG']['tl_pricelist_item']['description']  = array('Description', 'Please enter the .');
$GLOBALS['TL_LANG']['tl_pricelist_item']['published']    = array('Publish product', 'Make the item publicly visible on the website.');
$GLOBALS['TL_LANG']['tl_pricelist_item']['start']        = array('Show from', 'Do not show the item on the website before this day.');
$GLOBALS['TL_LANG']['tl_pricelist_item']['stop']         = array('Show until', 'Do not show the item on the website on and after this day.');


/**
 * Legends
 */
$GLOBALS['TL_LANG']['tl_pricelist_item']['title_legend']       = 'Title and code';
$GLOBALS['TL_LANG']['tl_pricelist_item']['status_legend']      = 'Status';
$GLOBALS['TL_LANG']['tl_pricelist_item']['meta_legend']        = 'Meta';
$GLOBALS['TL_LANG']['tl_pricelist_item']['unit_legend']        = 'Unit';
$GLOBALS['TL_LANG']['tl_pricelist_item']['description_legend'] = 'Description';
$GLOBALS['TL_LANG']['tl_pricelist_item']['publish_legend']     = 'Publish settings';


/**
 * Buttons
 */
$GLOBALS['TL_LANG']['tl_pricelist_item']['new']    = array('New Item', 'Create a new item');
$GLOBALS['TL_LANG']['tl_pricelist_item']['show']   = array('Item details', 'Show the details of item ID %s');
$GLOBALS['TL_LANG']['tl_pricelist_item']['edit']   = array('Edit item', 'Edit item ID %s');
$GLOBALS['TL_LANG']['tl_pricelist_item']['cut']    = array('Move item', 'Move item ID %s');
$GLOBALS['TL_LANG']['tl_pricelist_item']['copy']   = array('Duplicate item', 'Duplicate item ID %s');
$GLOBALS['TL_LANG']['tl_pricelist_item']['delete'] = array('Delete item', 'Delete item ID %s');
